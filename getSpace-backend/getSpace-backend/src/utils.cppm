...
namespace {
	std::tm get_local_tm(const std::time_t& t) {
		std::tm tm{};
		#ifdef _WIN32
	...
	std::wstring stringToWString(const std::string& str) {
		int size_needed = MultiByteToWideChar(CP_UTF8, 0, str.c_str(), str.size(), nullptr, 0);
		std::wstring wstr(size_needed, 0);
		MultiByteToWideChar(CP_UTF8, 0, str.c_str(), str.size(), &wstr[0], size_needed);
		return wstr;
	}
...
export namespace utils {
	void LOG(const std::string& message) {
		std::string data = "[" + current_datetime() + "] " + message + "\n";
